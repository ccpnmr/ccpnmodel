...
class Window(ccpnmodel.ccpncore.api.memops.Implementation.DataObject):
  ...
  _fieldNames = ('_ID', 'applicationData', 'className', 'fieldNames', 'inConstructor', 'isDeleted', 'metaclass', 'packageName', 'packageShortName', 'position', 'qualifiedName', 'serial', 'size', 'title', 'guiTask', 'modules', 'parent', 'root', 'topObject', 'windowStore',)
  ...
  def __init__(self, parent, **attrlinks):
    ...
    try:
      ...
      try:
        ...
        self.root._logger.error("in ccpnmr.gui.Window.Window.__init__")
        del dataDict['inConstructor']
        raise
  ...
  def empty(self):
    """
    Empty window by disconnecting all tasks shown in the window.
    """
    dataDict = self.__dict__
    if (dataDict.get('isDeleted')):
      raise ApiError("""%s.empty:
       called on deleted object""" % self.qualifiedName
      )

    for task in set(x.task for x in self.modules):
      task.disconnect()
  ...
  def findAllModules(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        ...
        directAttrs = frozenset(('_ID', 'details', 'name', 'windowId', 'guiTask', 'window',))
  ...
  r"""Window label. Defaults to 'Main' for serial == 1 and to 'W%s' % serial 
  for other windows.
  """)
...
class WindowStore(ccpnmodel.ccpncore.api.memops.Implementation.TopObject):
  ...
  def __init__(self, parent, **attrlinks):
    ...
    try:
      ...
      try:
        ...
        self.root._logger.error("in ccpnmr.gui.Window.WindowStore.__init__")
        del dataDict['inConstructor']
        raise
  ...
  def findAllWindows(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        (key, condition) = items[0]
        directAttrs = frozenset(('_ID', 'serial', 'title', 'guiTask', 'windowStore',))
        if (key in directAttrs):
          for v in currentValues:
            if (condition == v.__dict__.get(key)):
              result.add(v)
