...
class Chain(ccpnmodel.ccpncore.api.memops.Implementation.DataObject):
  ...
  _fieldNames = ('_ID', 'applicationData', 'chainCode', 'className', 'fieldNames', 'inConstructor', 'isDeleted', 'metaclass', 'packageName', 'packageShortName', 'qualifiedName', 'chain', 'haddockPartner', 'parent', 'residues', 'root', 'topObject',)
  ...
  def findAllResidues(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        ...
        directAttrs = frozenset(('_ID', 'flexibility', 'haddockSeqId', 'interaction', 'residueSeqId', 'chain', 'residue',))
...
class HaddockPartner(ccpnmodel.ccpncore.api.memops.Implementation.DataObject):
  ...
  def findAllChains(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        (key, condition) = items[0]
        directAttrs = frozenset(('_ID', 'chainCode', 'haddockPartner',))
        if (key in directAttrs):
          for v in currentValues:
            if (condition == v.__dict__.get(key)):
              result.add(v)
...
class Residue(ccpnmodel.ccpncore.api.memops.Implementation.DataObject):
  ...
  _packageName = 'utrecht.Haddock'
  _packageShortName = 'HADD'
  _fieldNames = ('_ID', 'applicationData', 'className', 'fieldNames', 'flexibility', 'haddockSeqId', 'inConstructor', 'interaction', 'isDeleted', 'metaclass', 'packageName', 'packageShortName', 'qualifiedName', 'residueSeqId', 'chain', 'parent', 'residue', 'root', 'topObject',)
