...
Autogenerated by PyFileApiGen on Sun May 15 02:19:07 2016
...
class Citation(ccpnmodel.ccpncore.api.memops.Implementation.DataObject):
  ...
  def findAllDataLists(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        (key, condition) = items[0]
        directAttrs = frozenset(('_ID', 'definition', 'details', 'name', 'serial', 'sf', 'unit', 'method', 'nmrProject',))
        if (key in directAttrs):
          for v in currentValues:
            if (condition == v.__dict__.get(key)):
              result.add(v)
  ...
  def findAllNmrProbes(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        (key, condition) = items[0]
        directAttrs = frozenset(('_ID', 'details', 'diameter', 'instrumentType', 'model', 'name', 'probeType', 'serial', 'serialNumber', 'contactPerson', 'instrumentStore', 'manufacturer',))
        if (key in directAttrs):
          for v in currentValues:
            if (condition == v.__dict__.get(key)):
              result.add(v)
  ...
  def findAllNmrSpectrometers(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        (key, condition) = items[0]
        directAttrs = frozenset(('_ID', 'details', 'instrumentType', 'model', 'name', 'nominalFreq', 'protonFreq', 'serial', 'serialNumber', 'contactPerson', 'instrumentStore', 'manufacturer',))
        if (key in directAttrs):
          for v in currentValues:
            if (condition == v.__dict__.get(key)):
              result.add(v)
  ...
  def findAllRatioShiftReferences(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        (key, condition) = items[0]
        directAttrs = frozenset(('_ID', 'atomGroup', 'indirectShiftRatio', 'isotopeCode', 'molName', 'referenceType', 'serial', 'unit', 'value', 'citation', 'nmrProject', 'shiftRatioCitation',))
        if (key in directAttrs):
          for v in currentValues:
            if (condition == v.__dict__.get(key)):
              result.add(v)
  ...
  def findAllSampleConditionSets(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        (key, condition) = items[0]
        directAttrs = frozenset(('_ID', 'details', 'name', 'serial', 'nmrProject',))
        if (key in directAttrs):
          for v in currentValues:
            if (condition == v.__dict__.get(key)):
              result.add(v)
...
class CitationStore(ccpnmodel.ccpncore.api.memops.Implementation.TopObject):
  ...
  _packageName = 'ccp.general.Citation'
  _packageShortName = 'CITA'
  _fieldNames = ('_ID', '_lastId', 'applicationData', 'className', 'createdBy', 'fieldNames', 'guid', 'inConstructor', 'isDeleted', 'isLoaded', 'isModifiable', 'isModified', 'isReading', 'lastUnlockedBy', 'metaclass', 'name', 'packageName', 'packageShortName', 'qualifiedName', 'activeRepositories', 'citations', 'memopsRoot', 'packageLocator', 'parent', 'root', 'topObject',)
...
class ConferenceCitation(Citation):
  ...
  _packageName = 'ccp.general.Citation'
  _packageShortName = 'CITA'
  _fieldNames = ('_ID', 'abstractNumber', 'applicationData', 'casAbstractCode', 'city', 'className', 'conferenceSite', 'conferenceTitle', 'country', 'details', 'doi', 'endDate', 'fieldNames', 'firstPage', 'inConstructor', 'isDeleted', 'keywords', 'lastPage', 'medlineUiCode', 'metaclass', 'packageName', 'packageShortName', 'pubMedId', 'qualifiedName', 'serial', 'startDate', 'stateProvince', 'status', 'title', 'year', 'authors', 'citationStore', 'dataLists', 'editors', 'experiments', 'methods', 'molSystems', 'molecules', 'nmrEntries', 'nmrEntriesPrimary', 'nmrProbes', 'nmrSpectrometers', 'parent', 'ratioShiftReferences', 'root', 'sampleConditionSets', 'shiftReferences', 'software', 'topObject',)
