...
class ChemAtomCharge(ccpnmodel.ccpncore.api.memops.Implementation.DataObject):
  ...
  _fieldNames = ('_ID', 'applicationData', 'className', 'fieldNames', 'inConstructor', 'isDeleted', 'metaclass', 'name', 'packageName', 'packageShortName', 'partialCharge', 'qualifiedName', 'serial', 'subType', 'validVegaTypes', 'chemAtom', 'chemCompCharge', 'chemCompVarCharges', 'parent', 'root', 'topObject',)
  ...
  def findAllChemCompVarCharges(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        ...
        directAttrs = frozenset(('_ID', 'descriptor', 'linking', 'chemCompCharge',))
...
class ChemCompCharge(ccpnmodel.ccpncore.api.memops.Implementation.TopObject):
  ...
  def findAllChemAtomCharges(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        (key, condition) = items[0]
        directAttrs = frozenset(('_ID', 'name', 'partialCharge', 'serial', 'subType', 'chemCompCharge',))
        if (key in directAttrs):
          for v in currentValues:
            if (condition == v.__dict__.get(key)):
              result.add(v)
...
with an 'any' attribute must belong to all the relevant Vars
  ...
  _packageName = 'ccp.molecule.ChemCompCharge'
  _packageShortName = 'CCCA'
  _fieldNames = ('_ID', 'applicationData', 'className', 'descriptor', 'fieldNames', 'inConstructor', 'isDeleted', 'linking', 'metaclass', 'packageName', 'packageShortName', 'qualifiedName', 'chemAtomCharges', 'chemCompCharge', 'chemCompVars', 'parent', 'root', 'topObject',)
