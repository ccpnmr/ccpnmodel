...
class ChemAtomCoord(ccpnmodel.ccpncore.api.memops.Implementation.DataObject):
  ...
  _fieldNames = ('_ID', 'applicationData', 'className', 'fieldNames', 'inConstructor', 'isDeleted', 'metaclass', 'name', 'packageName', 'packageShortName', 'qualifiedName', 'serial', 'subType', 'x', 'y', 'z', 'chemAtom', 'chemCompCoord', 'chemCompVarCoords', 'parent', 'root', 'topObject',)
  ...
  def findAllChemCompVarCoords(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        ...
        directAttrs = frozenset(('_ID', 'descriptor', 'linking', 'chemCompCoord',))
  ...
  def getChemAtom(self):
    ...
    result = ccpnmodel.cpncore.api.ccp.molecule.ChemComp.AbstractChemAtom.getByKey(self.root, (ccc.molType, ccc.ccpCode, self.name, self.subType))
    return result
...
class ChemCompCoord(ccpnmodel.ccpncore.api.memops.Implementation.TopObject):
  ...
  def findAllChemAtomCoords(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        (key, condition) = items[0]
        directAttrs = frozenset(('_ID', 'name', 'serial', 'subType', 'x', 'y', 'z', 'chemCompCoord',))
        if (key in directAttrs):
          for v in currentValues:
            if (condition == v.__dict__.get(key)):
              result.add(v)
...
relevant Vars
  ...
  _packageName = 'ccp.molecule.ChemCompCoord'
  _packageShortName = 'CCCO'
  _fieldNames = ('_ID', 'applicationData', 'className', 'descriptor', 'fieldNames', 'inConstructor', 'isDeleted', 'linking', 'metaclass', 'packageName', 'packageShortName', 'qualifiedName', 'chemAtomCoords', 'chemCompCoord', 'chemCompVars', 'parent', 'root', 'topObject',)
  ...
  def findAllChemCompVars(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        (key, condition) = items[0]
        directAttrs = frozenset(('_ID', 'descriptor', 'formalCharge', 'glycoCtCode', 'isAromatic', 'isDefaultVar', 'isParamagnetic', 'linking', 'nonStereoSmiles', 'stereoSmiles', 'varName', 'chemComp',))
        if (key in directAttrs):
          for v in currentValues:
            if (condition == v.__dict__.get(key)):
              result.add(v)
