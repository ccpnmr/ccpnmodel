...
creates the Residue and Atom objects corresponding to it. 
  ...
  def findAllChainGroups(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        (key, condition) = items[0]
        directAttrs = frozenset(('_ID', 'ccpnInternalData', 'name', 'molSystem',))
        if (key in directAttrs):
          for v in currentValues:
            if (condition == v.__dict__.get(key)):
              result.add(v)
...
class ChainGroup(ccpnmodel.ccpncore.api.memops.Implementation.DataObject):
  ...
  _packageName = 'ccp.molecule.MolSystem'
  _packageShortName = 'MOLS'
  _fieldNames = ('_ID', 'applicationData', 'ccpnInternalData', 'className', 'fieldNames', 'inConstructor', 'isDeleted', 'metaclass', 'name', 'packageName', 'packageShortName', 'qualifiedName', 'chains', 'molSystem', 'parent', 'root', 'topObject',)
  ...
  def __init__(self, parent, **attrlinks):
    ...
    try:
      ...
      try:
        ...
        self.root._logger.error("in ccp.molecule.MolSystem.ChainGroup.__init__")
        del dataDict['inConstructor']
        raise
  ...
  name = property(getName, setName, None,
  r"""Chain group name - key attribute
  """)
