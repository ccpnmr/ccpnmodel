...
Autogenerated by PyFileApiGen on Thu Jan 12 13:17:23 2017
...
class MolResidueData(Data):
  ...
  def findAllResidues(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        (key, condition) = items[0]
        directAttrs = frozenset(('_ID', 'ccpnInternalData', 'descriptor', 'details', 'linking', 'seqCode', 'seqId', 'seqInsertCode', 'chain', 'chainFragment',))
        if (key in directAttrs):
          for v in currentValues:
            if (condition == v.__dict__.get(key)):
              result.add(v)
